...
def calc_rgbps_iou(result, gt, protocol=1):
    ...
    iou = calc_iou_overlap(torch.tensor(result_non_zero2), torch.tensor(gt_non_zero2)).tolist()
...
def calc_rgbps_prec(result, gt, normalized=False, protocol=1):
    ...
    center_errors = calc_err_center(torch.tensor(result_non_zero2), torch.tensor(gt_non_zero2),
...
def calc_rgbps_seq_performace(results_boxes, gt_boxes, protocol=1):
    if len(results_boxes) != len(gt_boxes):
        return 0, 0, 0
